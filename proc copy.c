...
scheduler(void)
{
  ...
  for(;;){
    ...
    acquire(&ptable.lock);
    p = ptable.proc;
    int sched_policy = p->sched_policy;
    ...
    else if (sched_policy == 1)
    {
      ...
      struct proc *p_next = ptable.proc;
      for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
      {
        if (p->state != RUNNABLE)
          continue;
      ...
      swtch(&(c->scheduler), p_next->context);
      switchkvm();
